...
const Header = () => {
    ...
    return (
        <header className="w-full p-4 px-5 sm:px-10 flex items-center justify-between">
            ...
            <nav className="w-max py-3 px-8 border border-solid border-dark rounded-full font-medium capitalize hidden sm:flex items-center fixed top-6 right-1/2 translate-x-1/2 bg-light/80 backdrop-blur-sm z-50 custom-box-shadow">
                <Link href="/" className="mr-2">Home</Link>
                <Link href="/about" className="mx-2">About</Link>
                <Link href="/contact" className="mx-2">Contact</Link>
                <button onClick={() => setMode(mode === "light" ? "dark" : "light")} className={cx("w-6 h-6 ease ml-2 flex items-center justify-center rounded-full p-1", mode === "light" ? "bg-dark text-light" : "bg-light text-dark")} >
            ...
            <div className="hidden sm:flex items-center">
                ...
                <a href={siteMetadata.github} className="inline-block w-6 h-6 mr-4 custom-link-github dark:fill-light"><GithubIcon /></a>
            </div>
        </header>
    )
}
